...
CASE_TYPES: dict[str, dict[str, object]] = {
    "work_accident": {
        ...
        "diagnoses": ["좌측 발목 골절", "우측 손 압궤상", "요추 염좌", "우측 어깨 타박상"],
        "injury_parts": ["좌측 발목", "우측 손", "요추", "우측 어깨"],
    ...
    "commute": {
        ...
        "injury_parts": ["경추", "좌측 무릎", "우측 손목", "요추부"],
    ...
    "third_party": {
        ...
        "injury_parts": ["안면부", "우측 무릎", "흉부", "좌측 팔꿈치"],
    ...
    "survivor": {
        ...
        "injury_parts": ["전신", "두부", "흉부", "호흡기 및 전신"],
    ...
    "retreatment": {
        ...
        "diagnoses": ["우측 회전근개 재파열", "요추 통증 재발", "좌측 발목 불안정증", "우측 팔꿈치 수술 부위 신경병성 통증"],
        "injury_parts": ["우측 어깨", "요추", "좌측 발목", "우측 팔꿈치"],
...
def case_catalog() -> dict[str, tuple[AccidentCase, ...]]:
    ...
    for type_index, (case_type, config) in enumerate(CASE_TYPES.items(), start=1):
        ...
        for index in range(1, 121):
            ...
            # Each column at the same index describes one reviewed synthetic
            # scenario. Never independently mix diagnoses, tasks and events.
            scenario = (index * 5 + type_index) % len(config["events"])
            diagnosis = config["diagnoses"][scenario]
            place = config["places"][scenario]
            task = config["tasks"][scenario]
            event = config["events"][scenario]
            ...
            items.append(
                AccidentCase(
                    ...
                    injury_part=str(config["injury_parts"][scenario]),
